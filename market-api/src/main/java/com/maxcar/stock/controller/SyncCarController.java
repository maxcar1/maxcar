...
public class SyncCarController extends BaseController {
	...
	public InterfaceResult canAdd(@RequestBody CarChecks carCheck,HttpServletRequest r) throws Exception{
...
//		User user = getCurrentUser(request);
	...
	public InterfaceResult getCar(@RequestBody CarChecks carCheck,@PathVariable("type")Integer type,HttpServletRequest r) throws Exception{
		InterfaceResult interfaceResult = new InterfaceResult();
		User user = getCurrentUser(r);
			Map map;
			if (null == carCheck) {
			...
			if (null != carCheck.getStaffId() && "" != carCheck.getStaffId()) {
				...
				Integer value = 0;
				if(carCheck.getType() == 2){//市场员工车
					Configuration configuration1 = new Configuration();
					configuration1.setMarketId(user.getMarketId());
					configuration1.setConfigurationKey("staff_car_market");
					List<Configuration> configurationList = configurationService.searchConfiguration(configuration1);
					value = Integer.parseInt(configurationList.get(0).getConfigurationValue());
				}
				if(carCheck.getType() == 3){//商户员工车
					Configuration configuration1 = new Configuration();
					configuration1.setMarketId(user.getMarketId());
					configuration1.setConfigurationKey("staff_car_tenant");
					List<Configuration> configurationList = configurationService.searchConfiguration(configuration1);
					value = Integer.parseInt(configurationList.get(0).getConfigurationValue());
				}
				if (counts >= value) {
			...
			if (null != carByVin) {
				if(carByVin.getCarType() ==2 ){
					interfaceResult.InterfaceResult600("该vin码属于员工车，不能进行商品车绑定");
					return interfaceResult;
				}
				if(carByVin.getCarType() ==3){
					interfaceResult.InterfaceResult600("该vin码属于商户车，不能进行商品车绑定");
					return interfaceResult;
				}
...
//	}
	...
	public InterfaceResult modifyTenant(@RequestBody CarTenantVo carTenantVo,HttpServletRequest request) throws Exception{
		...
		if (StringUtils.isNotBlank(carTenantVo.getTenant())) {
			...
			GetCarSpaceAndOfficeByMarketIdOrAreaIdRequest r1 = new GetCarSpaceAndOfficeByMarketIdOrAreaIdRequest();
			r1.setMarketId(user.getMarketId());
			r1.setTenantId(tenant.getId());
			GetCarTotalByMarketIdOrTenantIdOrAreaIdResponse r2 = propertyContractService.getCarTotalByMarketIdOrTenantIdOrAreaId(r1);
			if(r2 == null){
				interfaceResult.InterfaceResult600("请联系管理员，配置相关参数");
				return interfaceResult;
			}
			if(r2!= null && r2.getCarTotal() ==0){
				interfaceResult.InterfaceResult600(r2.getMessage());
				return interfaceResult;
			}
			int number = 0;
			if(r2 != null){
				number = r2.getCarTotal();//总车位数
			}
			Configuration c = new Configuration();
			c.setMarketId(user.getMarketId());
			c.setConfigurationKey("car_num");
			...
			Car tenantIdCar = new Car();
			tenantIdCar.setTenant(tenant.getId());
