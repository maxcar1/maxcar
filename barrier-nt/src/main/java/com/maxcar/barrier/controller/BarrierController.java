...
public class BarrierController {
    ...
    public void upload(@RequestBody Car car,HttpServletRequest request) {
        try{
            Car carnew = new Car();
            carnew.setStockStatus(car.getStockStatus());
            carnew.setId(car.getId());
            if (null != car.getTenant() && car.getTenant() != "") {
                carnew.setTenant(car.getTenant());
            }
            carService.updateCarStatus(carnew);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
