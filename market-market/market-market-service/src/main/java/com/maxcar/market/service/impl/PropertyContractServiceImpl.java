...
public class PropertyContractServiceImpl extends BaseServiceImpl<PropertyContract, String> implements PropertyContractService {
    ...
    public GetCarTotalByMarketIdOrTenantIdOrAreaIdResponse getCarTotalByMarketIdOrTenantIdOrAreaId(GetCarSpaceAndOfficeByMarketIdOrAreaIdRequest request) throws Exception {
        ...
        if (!StringUtils.isEmpty(request.getTenantId())) {
            ...
            if (null != propertyContractDetailList && !propertyContractDetailList.isEmpty()) {
                for (PropertyContractDetail propertyContractDetail : propertyContractDetailList) {
                    ...
                    if (Magic.CONTRACT_CATEGORY_CAR_SPACE.equals(propertyContractDetail.getContractCategory())) {
                        ...
                        // 租赁物业是办公室
                        office += number;
                    } else if (Magic.CONTRACT_CATEGORY_TEMPORARY_CAR_SPACE.equals(propertyContractDetail.getContractCategory())) {
                        // 租赁物业是临时车位
                        carTotal += number;
                        temporaryCarSpace += number;
                    } else if (Magic.CONTRACT_CATEGORY_WAREHOUSE.equals(propertyContractDetail.getContractCategory())) {
                        // 租赁物业是仓库
                        warehouse += number;
                    } else if (Magic.CONTRACT_CATEGORY_OTHER.equals(propertyContractDetail.getContractCategory())) {
                        // 租赁物业是其他
                        other += number;
                    }
                }
                ...
                GetPropertyContractDetailListRequest listRequest = new GetPropertyContractDetailListRequest();
                listRequest.setTenantId(request.getTenantId());

                List<PropertyContractDetail> list = propertyContractDetailMapperService.getPropertyContractDetailList(listRequest);
            ...
            response.setTemporaryCarSpace(temporaryCarSpace);
     ...
     **/
    @Override
    public String addPropertyContract(AddContractRequest request) {
    ...
    public boolean updatePropertyContract(UpdatePropertyContractRequest request) {
        ...
        propertyContract.setRemark(request.getRemark());
        // 修改合同信息
    ...
    public GetPropertyContractResponse getPropertyContract(GetPropertyContractRequest request) {
        ...
        userTenantById.setMarketId(propertyContract.getMarketId());
        ...
        if (null != userTenant) {
            ...
            userTenantPack.setTenantAddres(userTenant.getTenantAddress());
            userTenantPack.setTenantPhone(userTenant.getTenantPhone());
        }
