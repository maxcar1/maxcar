...
//    	carBaseMapper.
    ...
    @Override
    public List<Configuration> carLevelGroup(String marketId,String type) {
        return carBaseMapper.selectCarGroup(marketId,type);
    }
...
//            carBase.setGearBox("%" + carBase.getGearBox() + "%");
    ...
    public PageInfo selectCarList(SearchCarRequest carRequest) {
        ...
        if (StringUtils.isNotBlank(carRequest.getSeriesCode()) && carRequest.getSeriesCode().startsWith("brand")){
            carRequest.setBrandCode(carRequest.getSeriesCode());
            carRequest.setSeriesCode("");
        }

        carRequest.setBeginPageIndex((carRequest.getCurrentPage() - 1) * carRequest.getPageSize());
        ...
        for (CarBase car : list) {
            ...
            carBaseVo.setSrc(carPicMapper.getPicSrcByCarId(carBaseVo.getId()));
    ...
    @Override
    public CarBase getCarBaseByVin(String vin) {
    ...
    public List<VehicleBrandVo> getBrandTop10(String marketId) {
        ...
        for(VehicleBrandVo carBrand : vehicleBrandList){
            ...
            carBrand.setChildren(chooseList);
        }
